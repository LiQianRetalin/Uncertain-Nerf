...
## 16. 用户确认V3 30k完成，服务器筛选标签NO_GO

用户提供GPU0（UUID `GPU-e75e3686-bd4e-bbaf-9417-28a4a31d5744`）状态为`TRAIN_COMPLETE / exit_code=0 / last_step=29999`。包装PID2306264、child PID2306856；命令使用固定V3配置和原静态缓存，`--v3-stop-step 29999`，未传checkpoint/resume。标准checkpoint存在且可加载，SHA-256为`14563fbbcfbb722879929be31fd19ad5e6a02939e8d2409e6a6855ce909d417c`，Gaussian数2266597。数量低于2312002上限45405个，比历史标准RU少7600个；该门通过不证明质量恢复。

子进程墙钟1244.8311860628892秒，不能将其与历史Parent的训练段时间直接相除。用户随后启动`eval-v3`，包装PID2319513；report命令返回`QUALITY_RECOVERY_FAIL / NO_GO`。依据当前分类逻辑，该标签说明至少一个固定质量恢复门为false，但不能仅凭标签判断哪个指标失败、是否相对Parent下降或监督是否激活。全程Q激活与最终固定训练视图证据不能沿用600步Q=0结论。

本次仅更新测量记录并提供简短JSON字段回读命令，不改训练代码，不要求同步或重复评测。下一步读取已存在的eval-v3状态、screening_result关键门/指标/ROI/成本和30k训练检查/证据；完成负结果报告后停止，不自动重训、调阈值或开展下一算法。

## 17. 完整回读核对与本轮关闭

用户最新附件包含完整的紧凑JSON输出。命令回显有重复拼接，但实际输出可逐项解析；`eval_state=EVAL_COMPLETE / exit_code=0 / error=null`，`missing=[] / invalid=[]`。只解析17个已知输出字段，原样保留数值，并将来源路径及SHA-256、原始字段和独立复算结果保存到`E:/7-DataSet/ru_part_v3/server_readback_20260909.json`。该文件标记为USER_PASTED_SERVER_READBACK，remote_files_directly_read=false。

V3整体指标为26.808757781982422 / 0.8582214117050171 / 0.09035919606685638，相对Parent变化为+0.1581859588623047 / +0.0009161829948425293 / −0.0001471191644668579。DSC07988 PSNR为14.745124816894531，相对Parent提高0.6647405624389648 dB。四项固定质量恢复门全FAIL，三项Parent质量保护门全PASS。按固定门复算差距，PSNR还差0.757970218 dB、SSIM还差0.010735588、LPIPS仍高0.017208196、DSC07988 PSNR还差4.746176183 dB；与服务器门结果完全一致。

原固定ROI共77812像素，历史定义及两份来源数组SHA均有记录。误差均值0.5177793503→0.4485192001，降低13.376383%；alpha均值0.2006936669→0.2937479615；低alpha像素58168→48358，减少9810，低alpha面积占ROI比例74.7545366%→62.1472266%，下降12.6073099个百分点。必须使用同一ROI分母，不能把相对面积下降16.864943%写成16.86个百分点；也不能将局部alpha改善解释为几何已正确或孔洞已修复。

30k训练检查为ACTIVE，active_samples=2265，added_loss_sum=0.0874202772974968。代码逐次计数`q.amax()>0`，因此2265指有非零Q的更新次数，占step500..29999共29500步约7.677966%，不是激活像素数。Gaussian backward30000、训练raster50000、七项禁用计数全0、head更新29400，与固定路径和reset暂停调度一致。梯度检查仅在step500与599执行，日志中两个时点均通过；不能宣称提供了step29999梯度检查。

最终两张固定训练图的证据scope=FINAL_TRAINING_STATE。DSC07987的C非零比例2.002423%，Mask拒绝14.886919%，但Q及最高20%残差区域Q覆盖均为0。DSC07989的C非零比例2.610420%，Mask拒绝0.843614%，Q非零像素比例0.071961%，最高20%残差区域Q覆盖0.289129%，mean_Q约0.000095163。聚合ACTIVE来自至少一张图激活，不表示训练侧难区覆盖充分；可判定诊断视图的有效覆盖有限，不能证明其为整体失败唯一原因或外推到所有训练图。

数量2266597、独立单raster、FPS比0.999022491均PASS；V3 FPS143.230354848，与Parent143.370500769基本持平。V3训练段1212.198882580秒、峰值4.667714596 GiB；原Parent训练段1347.454995632秒、峰值4.680006027 GiB，训练GPU/记录方式不同，time gate=null，未宣称提速。缓存准备0.580098892秒、原缓存构建561.269638196秒，训练段加原缓存成本1773.468520776秒分别保留，不当成本轮新发生的总墙钟。

最终报告已重写为完整结果；本地验证了JSON字段齐全、评测成功、各质量门复算一致、ROI分母一致、关键调用计数与禁用项一致。此次没有修改训练/评测代码，无需再跑单元测试或CUDA作业。实验只覆盖Garden单seed开发筛选，统计显著性、跨场景、因果训练重放和静态标签正确性仍未被证明。按协议保持QUALITY_RECOVERY_FAIL / NO_GO并停止，保留全部正向小幅增益和负结果，不自动启动进一步实验。
